...
/mob/living/carbon/human/verb/issue_order()
	...
	switch(command_aura)
		if("move")
			message = pick(";ПОШЕВЕЛИВАЙТЕСЬ!", ";ВПЕРЕД, ВПЕРЕД, ВПЕРЕД!", ";ДВИЖЕМСЯ!", ";ШЕВЕЛИТЕСЬ!", ";БЫСТРЕЕ!")
			say(message)
		if("hold")
			message = pick(";В УКРЫТИЕ!", ";ДЕРЖАТЬ ЛИНИЮ!", ";ДЕРЖАТЬ ПОЗИЦИЮ!", ";ВСЕМ СТОЯТЬ НА МЕСТЕ!", ";СТОЯТЬ И СРАЖАТЬСЯ!")
			say(message)
		if("focus")
			message = pick(";СФОКУСИРОВАТЬ ОГОНЬ!", ";ПРИЦЕЛИТЬСЯ!", ";ОГОНЬ ПО ЦЕНТРУ!", ";ПРИЦЕЛЬНЫМИ ОЧЕРЕДЯМИ!", ";ПРИЦЕЛЬНЫЙ ОГОНЬ!")
			say(message)
