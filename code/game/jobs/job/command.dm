...
While you support Weyland-Yutani, you report to the UPP High Command, not the corporate office.
...
Godspeed, commander!"}
	...
	announce_entry_message(mob/living/carbon/human/H)
		..()
		sleep(15)
		if(H && H.loc && flags_startup_parameters & ROLE_ADD_TO_MODE) captain_announcement.Announce("All hands, Marshal [H.real_name] on deck!")
...
The soldiers need a leader to inspire them and lead them to victory. You'll settle for telling them which side of the gun the bullets come from.
...
/datum/job/command/executive
	...
	generate_entry_message(mob/living/carbon/human/H)
		. = {"You are second in command aboard the ship, and are in next in the chain of command after the marshal.
